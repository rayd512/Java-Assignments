...
public class SkipList<K extends Comparable<K>, V> {
	...
	private Node header = new Node(null, null, 1);
	...
	public void insert(K key, V value) {
		...
		for(int i = this.level; i >= 0; i--) {
			while(current.forwards.get(i).key != null && current.forwards.get(i).key.compareTo(key) < 0) {
	...
	public V remove(K key) {
		...
		if(current.key != null && current.key == key) {
			...
			// while (level > 0 && this.header.forwards.get(level) == 0)
			// 	level--;
