...
public class ArrayListExample {
     ...
     */
    public static <T extends TwoDShape<T>> double total_area(ArrayList<T> shapes) {
        double total = 0;
        for (T shape : shapes)
            total += shape.area();
        return total;
    ...
    public static <T extends GeometricShape<T>> void describe_all(ArrayList<T> shapes) {
        for(T shape : shapes)
            shape.describe();
    ...
    public static <T extends GeometricShape<T>> ArrayList<T> supersize_list(ArrayList<T> shapes) {
        ArrayList<T> supersized = new ArrayList<T> ();
        for (T shape : shapes) 
            supersized.add(shape.supersize());
        return supersized;
    }
